...
def create_superposed_traces_figure(fig_data, batch_file_string):
    ...
    fig_width = layout['left_margin'] + \
        (no_of_conditions * 4) + layout['right_margin']    
    ...
    for (i, rfs) in enumerate(results_file_strings):
        ...
        plot_index = plot_index + 1
        ax[plot_index].plot(d['time'], d['vent_chamber_radius'])

        plot_index = plot_index + 1
        ax[plot_index].plot(d['time'], d['vent_chamber_height'])
        ...
        plot_index = plot_index + 1
        ax[plot_index].plot(d['time'], d['muscle_length'])
        
        plot_index = plot_index + 1
        ax[plot_index].plot(d['time'], d['fs_hs_stress_titin'], label='Titin')
        ax[plot_index].plot(d['time'], d['fs_hs_stress_extracellular'], label='Extracellular')
    ...
    # Set up for labels and legends
    bbox = (formatting['legend_bbox_to_anchor'][0],
            formatting['legend_bbox_to_anchor'][1])
    
    plot_index = 0
    for i,lab in enumerate(y_axis_labels):
        
        if (i < (len(y_axis_labels)-1)):
            ax[plot_index].get_xaxis().set_visible(False)
    
        ax[plot_index].set_ylabel(lab,
                                  loc='center',
        ...
        if not (ax[plot_index].get_legend_handles_labels() == ([], [])):
